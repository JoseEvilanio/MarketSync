...
export default function ImportarXmlModal({ open, onClose, onSuccess }: Props) {
  ...
  const { mutate: importar, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(`NF-e ${data.numero}-${data.serie} importada com sucesso!`);
      setArquivo(null);
      setProgresso(0);
      onSuccess(data);
    },
  ...
  function handleFile(file: File) {
    if (!file.name.toLowerCase().endsWith('.xml')) {
      toast.error('Selecione um arquivo .xml'); return;
    }
    setArquivo(file);
  }
  ...
  function handleConfirmar() {
    if (!arquivo) { toast.error('Selecione um arquivo XML primeiro'); return; }
    importar(arquivo);
  }

  function handleClose() {
    if (isPending) return;
    setArquivo(null); setProgresso(0); onClose();
  }
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Importar NF-e (XML)" size="lg">
      <div className="space-y-4">
        ...
        {/* Zona de upload */}
        <input id="xml-file-input" ref={fileRef} type="file" accept=".xml"
        ...
        {!arquivo ? (
          ...
          >
            ...
            <p className="text-xs text-on-surface-variant">Aceita arquivos <code className="bg-surface-variant px-1 rounded">.xml</code> até 10 MB</p>
          </label>
        ) : (
        ...
        <div className="flex justify-end gap-3 border-t border-outline-variant pt-4">
          <button type="button" onClick={handleClose} disabled={isPending} className="btn-outline">
            Cancelar
          </button>
          <button type="button" onClick={handleConfirmar} disabled={!arquivo || isPending}
